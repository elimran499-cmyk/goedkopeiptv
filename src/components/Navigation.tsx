...
export default function Navigation({ activeTab, setActiveTab, darkMode, toggleDarkMode }: NavigationProps) {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50">
      ...
      {/* Urgency bar */}
      <div className="bg-[#ef4444] text-white text-xs font-bold text-center py-1.5 px-4 flex items-center justify-center gap-2">
        ...
        <button
          onClick={() => window.open(`https://wa.me/447449708976?text=${encodeURIComponent("Hallo, ik wil graag gebruik maken van de tijdelijke aanbieding bij goedkopeiptv.")}`, "_blank")}
      ...
      <div className="bg-[#0d1117] border-b border-white/5">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-6">
              ...
              return (
                <button
                  ...
                  className={`relative px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-150 cursor-pointer whitespace-nowrap ${
                    isActive
                      ? "text-white after:absolute after:bottom-0 after:left-3 after:right-3 after:h-0.5 after:bg-[#ef4444] after:rounded-full"
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              onClick={() => window.open(`https://wa.me/447449708976?text=${encodeURIComponent("Hallo, ik wil graag een IPTV pakket bestellen.")}`, "_blank")}
            ...
            <button
              ...
              className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:text-white transition cursor-pointer"
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="md:hidden bg-[#0d1117] border-b border-white/10 px-4 py-3 flex flex-col gap-1"
          >
            {navItems.map((item) => {
              const isActive = activeTab === item.id;
              return (
            ...
            <button
              onClick={() => { setMenuOpen(false); window.open(`https://wa.me/447449708976?text=${encodeURIComponent("Hallo, ik wil graag een IPTV pakket bestellen.")}`, "_blank"); }}
